...
uint32_t latitude, longitude, SatellitiesNum;// lat, lon value
double latvalue1, latvalue2, lonvalue1, lonvalue2;
double latvalue3, latvalue, relaspeed, reladir;
double lonvalue3, lonvalue, Height;
char status;
...
Serial.print( ", " );
    Height = parsedecimal( parseptr );
    parseptr = strchr( parseptr, '.' ) + 1;
Serial.print( parseptr );    
    Height += ( parsedecimal( parseptr ) / 10.0 );
...
Serial.print( ", " );
    tmp = parsedecimal( parseptr );
    parseptr = strchr( parseptr, '.' ) + 1;
    relaspeed = tmp + ( ( parsedecimal( parseptr ) / 100.0 ) ) * 0.51444;
    ...
    parseptr = strchr( parseptr, ',' ) + 1;
    reladir = parsedecimal( parseptr );
    parseptr = strchr( parseptr, '.' ) + 1;
    reladir += ( parsedecimal( parseptr ) / 100.0 );

    Serial.print( "Relation ground direction: " );
    Serial.print( reladir );
    Serial.print( "(deg)" );
    Serial.println();
